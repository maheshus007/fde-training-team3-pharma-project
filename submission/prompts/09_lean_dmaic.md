...
**Consolidate** the Lean / DMAIC operating spine before tasks and coding. Prior prompts (01–08) already produced thin artefact `02_DMAIC_WORKBOOK.md` files — **do not ignore them**. Start from those lenses + Prompt 01 early waste signals, then produce the **full** DOWNTIME + AI-waste registers and a complete DMAIC plan.
...
- **artefact `02_DMAIC_WORKBOOK.md` from Prompts 01–08** are available (or gaps noted).
...
Roll up prior DMAIC notes inside artefact `02_DMAIC_WORKBOOK.md` (do not create `lens_rollup.md`):

- Table of prior artefact `02_DMAIC_WORKBOOK.md` paths (01–08) with DMAIC focus and top findings  
...
Record structural reopen gate inside artefact `11_ADR_REGISTER.md` / `10_C4_ARCHITECTURE.md` (do not create `structural_reopen.md`):
...
- [ ] Structural reopen gate is recorded in artefacts `10`/`11` with decision `cleared` (or reopen completed).
...
- Do not ignore prior artefact `02_DMAIC_WORKBOOK.md` files — consolidate them.
...
Do **not** create separate waste_register/dmaic_plan files. Consolidate Lean/DMAIC into:

- `submission/artefacts/02_DMAIC_WORKBOOK.md` (complete)
- Feed measurable content into `22_EVALUATION_SCORECARD.md`, `23_TOKEN_FINOPS.md`, `24_RELIABILITY_OBSERVABILITY.md` as applicable (finish in Prompt 12)
